...
glob_latency = []
glob_throughput = []
async def custom_perform_http_request(
    client: HttpClient,
    url: str,
    data: Optional[str],
    include: bool,
    ...
    while True:
        ...
        if (end-start) > cycle_time:
            ...
            with open('metrics_scenario.json', 'a') as outfile:
                json_obj = {
                    str(datetime.now(pytz.timezone("UTC"))): [mean_latency, mean_throughput, max_latency, p_loss_rate]}
                json.dump(json_obj, outfile)
                outfile.write('\n')
            print(latency_color + str(datetime.now(pytz.timezone("UTC"))) + "," + str(
                mean_latency) + Style.RESET_ALL + "," + str(mean(throughput_list)) + "," + str(
                max_latency) + "," + ploss_color + str(p_loss_rate) + Style.RESET_ALL)
            latency_list = []
            throughput_list = []
            p_tot = 0
            p_loss = 0
            ...
            if data is not None:
                ...
                http_events = await client.get(url)
                method = "GET"
            elapsed = time.time() - latency_start
            latency = elapsed
            latency_list.append(latency)
            if p_loss_rate:
                throughput_list.append((536 / latency) * (1 / math.sqrt(
                    p_loss_rate)))  # Mathis equation:(MSS in bytes/RTT) * [1/sqrt(p_loss] MSS=Maximum Segment Size= 356 bytes
            else:
